...
class EditPageState extends State<EditPage> {
  ...
  void _initializeContactData() {
    ...
    // 1. Проверяем что arguments не null и это String
    if (arguments == null || arguments is! String) {
      _showMessage(context: context, message: 'Ошибка загрузки контакта');
      Navigator.pop(context);
      return;
    }

    // 2. Сохраняем ID
    _contactId = arguments;

    // 3. Находим контакт и заполняем поля
    final provider = Provider.of<MyProvider>(context, listen: false);
    final contact = provider.getContactById(_contactId);

    if (contact == null) {
      _showMessage(context: context, message: 'Контакт не найден');
      Navigator.pop(context);
      return;
    }

    // 4. Заполняем поля данными
    setState(() {
      _nameController.text = contact.name;
      _surnameController.text = contact.surname;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(
        title: const Text('Редактирование контакта'),
        centerTitle: true,
        leading: IconButton(
  ...
  void _saveContact() {
    if (_formKey.currentState!.validate()) {
      ...
      _showMessage(context: context, message: 'Контакт изменен');
